...
<div class="row">
    <form method="POST" action="/tulis/{{$berita->id}}/update" enctype="multipart/form-data">
      {{csrf_field()}}
    <div class="col-md-12">
        <div class="mb-3">
            ...
            <div class="col">
              <select class="form-select" name="kategori">
                @foreach ($ktgr_array as $array)
                    <option value="{{$array->id}}" @php if($berita->kategori==$array->id){echo"selected";} @endphp>{{$array->kategori}}</option>
                @endforeach
              </select>
            </div>
        </div>
      ...
      <div class="form-group mb-3 row">
          ...
          <div class="col">
            <select class="form-select" name="status">
              <option value="1" @php if($berita->status=="1"){echo"selected";} @endphp>Publik</option>
              <option value="2" @php if($berita->status=="2"){echo"selected";} @endphp>Draft</option>
            </select>
          </div>
      </div>
  </div>
    ...
    <div class="col-12">
        ...
        {{-- <div ></div> --}}
        <textarea id="example" name="isi">{!! $berita->isi !!}</textarea>
